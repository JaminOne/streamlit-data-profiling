...
if check_password():
    ...
    if query_table_name:
        ...
        # duplicate row based on column selections
        st.write("#### Duplicate Row By Selecting Columns")
        dropdown_selection = df.columns
        #create st.form to reduce page refreshes every time user select a column
        with st.form("choose_column"):
            options = st.multiselect("Select Columns or Keys: ", dropdown_selection)
            submitted = st.form_submit_button("Submit")
        if submitted:
            if options:
                st.write("Based on selected columns:", ", ".join(options), ". Below is the duplicated rows:")
                st.write(df[df.duplicated(subset=options, keep=False)])
        # distribution graph
